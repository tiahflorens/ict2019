...
def write_results(prediction, confidence, num_classes, nms=True, nms_conf=0.4):
    ...
    for ind in range(batch_size):  # N *(22743, 85), processing per image.
        # select the single image from the batch
        ...
        # 22743 proposals  , 80 classes

        max_conf = max_conf.float().unsqueeze(1)  # (22743, 1)
        max_conf_score = max_conf_score.float().unsqueeze(1)  # (22743, 1)

        seq = (image_pred[:, :5], max_conf, max_conf_score)
        image_pred = torch.cat(seq, 1)  # # (22743, 7)

        # Get rid of the zero entries
        non_zero_ind = (torch.nonzero(image_pred[:, 4]))  # 4-th element is confidence
        image_pred_ = image_pred[non_zero_ind.squeeze(), :].view(-1, 7)  # x,y,w,h,c, cls_score, cls_index
        ...
        for cls in img_classes:
            if cls == 0 or cls == 2: # person or car
                ...
                if nms:
                    """
                    nms for each frame, for each class
                    """
                ...
                if not write: # initialize output matrix; concat { (B,1)+ (B,7) } --> ( B, 1+7 )
                    output = torch.cat(seq, 1)
                    write = True
                else:
                    out = torch.cat(seq, 1)  # { (B,1)+ (B,7) } --> ( B, 1+7 )
                    output = torch.cat((output, out))  # concat --> N * ( B, 1+7 )
                num += 1
